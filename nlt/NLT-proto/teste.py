...
class TestWindow(QMainWindow): 
    def __init__(self):
        super().__init__()
        ###### Variables ######
        self.pontuacao_atual = 0 # inicia a pontuação a zero
        self.fase_atual = 2      # controla o bloco atual (0 a 4)
        self.regras_blocos = [
            {"nome": "familiarizacao", "tem_feedback": False, "ganho": 0, "perda": 0},
            {"nome": "linha_de_base", "tem_feedback": False, "ganho": 0, "perda": 0},
            {"nome": "motivacional_simetrico", "tem_feedback": True, "ganho": 10, "perda": -10},
            {"nome": "assimetria_punicao", "tem_feedback": True, "ganho": 10, "perda": -20},
            {"nome": "assimetria_recompensa", "tem_feedback": True, "ganho": 20, "perda": -10}
        ]
        ...
        texto_pausa_1 = QLabel('Muito bem.\nAgora treinaremos só com os quadrantes de BAIXO (números).\n\n(Lembrando)\nNÚMERO:\nB: par, N: ímpar', self.central_widget)
        texto_pausa_2 = QLabel('Agora misturaremos tudo. O estímulo pode aparecer em QUALQUER quadrante.\n\nCIMA: Letra (B: vogal, N: consoante)\nBAIXO: Número (B: par, N: ímpar)', self.central_widget)
        texto_pausa_3 = QLabel('FASE DE TREINO CONCLUÍDA.\nO teste vai começar.\nTente responder o mais rápido e corretamente possível.', self.central_widget)
        texto_pausa_4 = QLabel('ATENÇÃO!\nA partir de agora, as suas respostas VALEM PONTOS.\n\nAcertos ganham pontos (+10)\nErros perdem pontos (-10).', self.central_widget)
        texto_pausa_5 = QLabel('A REGRA MUDOU.\nAgora, os erros custam mais caro.\n\nACERTOS GANHAM (+10)\nERROS PERDEM (-20).', self.central_widget)
        texto_pausa_6 = QLabel('A regra mudou novamente!\nAgora, os acertos valem muito mais.\n\nAcertos ganham (+20)\nErros perdem (-10).', self.central_widget)
        
        self.block_labels = [texto_pausa_1, texto_pausa_2, texto_pausa_3, texto_pausa_4, texto_pausa_5, texto_pausa_6]
        ...
        # label do saldo total de pontos
        self.score_label = QLabel(f'Pontuação: {self.pontuacao_atual}', self.central_widget)
        self.score_label.setAlignment(Qt.AlignTop | Qt.AlignRight)
        self.score_label.setStyleSheet("color: yellow; font-weight: bold;")
        self.score_label.setFont(labelF)
        # posicionamento no topo
        self.score_label.setGeometry(0, 20, self.screen_width - 50, 100) 
        self.score_label.setHidden(True) # invisivel, fases 0 e 1 não têm pontos

        # label feedback
        self.feedback_label = QLabel('', self.central_widget)
        self.feedback_label.setAlignment(Qt.AlignCenter)
        self.feedback_label.setFont(QFont("Arial", 60, QFont.Bold))
        self.feedback_label.setGeometry(0, 0, self.screen_width, self.screen_height)
        self.feedback_label.setHidden(True)
    ...
    def next_page(self):
        ...
        if self.block_page:
            ...
            # --- ATUALIZAÇÃO DA FASE FAPESP ---
            # Aqui ligamos a pausa visual à fase lógica das regras!
            if self.block_index == 3:
                self.fase_atual = 1 # Entra na Linha de Base
            elif self.block_index == 4:
                self.fase_atual = 2 # Entra no Motivacional Simétrico
                self.score_label.setHidden(False) # Liga o placar de pontos no ecrã!
            elif self.block_index == 5:
                self.fase_atual = 3 # Entra na Assimetria Punição
            elif self.block_index == 6:
                self.fase_atual = 4 # Entra na Assimetria Recompensa
            # ----------------------------------
        ...
        elif self.q_is_pressed:
            # so it will only work after you quit explanation screen
            time.sleep(0.2)
    ...
    def trial_builder(self):
        ...
        regras = self.regras_blocos[self.fase_atual]
        if self.got_right:
            self.user_answers.append("Correct")
            
            if regras["tem_feedback"]:
                self.pontuacao_atual += regras["ganho"]
                self.feedback_label.setText(f"+{regras['ganho']}")
                self.feedback_label.setStyleSheet("color: green;")
                self.feedback_label.setHidden(False)
        
        elif self.got_wrong:
            self.user_answers.append("Wrong")
            
            if regras["tem_feedback"]:
                self.pontuacao_atual += regras["perda"]
                self.feedback_label.setText(f"{regras['perda']}")
        ...
        if regras["tem_feedback"]:
            # atualiza o placar total
            self.score_label.setText(f'Pontuação: {self.pontuacao_atual}')
            self.score_label.setHidden(False)
            
            # Desativa as teclas por 1 segundo para o utilizador poder ler os pontos que ganhou/perdeu
            self.b_shortcut.setEnabled(False)
            self.n_shortcut.setEnabled(False)
            QTimer.singleShot(1000, self.limpar_feedback_e_continuar) # Espera 1 seg e avança
            ...
            # para fases SEM feedback
            self.got_right = False
            if self.got_wrong:
                self.got_wrong = False
                self.wrong_timer.start() # Usa o timer original de 3 segundos para o erro
                self.n_shortcut.setEnabled(False)
                self.b_shortcut.setEnabled(False)
            else:
    ...
    def limpar_feedback_e_continuar(self):
        """ esconde os pontos e avança para a próxima tentativa """
        self.feedback_label.setHidden(True)
        self.b_shortcut.setEnabled(True)
        self.n_shortcut.setEnabled(True)
